...
const updateStory=async(req,res)=>{
    ...
    const updatedStory=await Story.findIdAndUpdate(_id,story,{new:true});
    res.json(updatedStory);
}

const deleteStory=async(req,res)=>{
    const {id} =req.params;
    if(!mongoose.Types.ObjectId.isValid(_idid)){
        return res.status(404).send("This id doesnt belong to any story");
    }
...
const likeStory=async(req,res)=>{
    ...
    const index=story.likes.g=findIndex(id=> id===String(req.userId));
